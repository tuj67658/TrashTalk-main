    ...
    <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === "ios" ? "padding" : "height"}>
      <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
        <ImageBackground source={require('./images/RegistrationBackground.png')} style={styles.image}>
          ...
          <TouchableOpacity style={(email.length > 0 && password.length > 0) ? styles.primaryButton : styles.primaryButtonDisabled} onPress={() => doSignIn()}>
            <Text style={(email.length > 0 && password.length > 0) ? styles.primaryButtonText : styles.primaryButtonDisabledText}>
              {"Sign In"}
            </Text>
